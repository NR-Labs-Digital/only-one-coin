...
- **Catálogo sai do ar, não some (decisão 22/09/2026).** Curso, plano, turma e período são **aposentados** (`deleted_at`), nunca apagados — a matrícula que aconteceu neles continua apontando pra eles, e o livro de matrículas continua mostrando tudo. Aposentar **não é bloqueado por matrícula viva**: segue o mesmo princípio do docente saindo do quadro (item abaixo) — avisa, não trava. A resposta diz quantas matrículas ainda estavam de pé (não aposentadas e com vaga não devolvida), porque o curso cuja última turma ainda roda é exatamente o que alguém precisa parar de vender. Não cascateia em escrita e não precisa: as leituras do catálogo filtram cada tabela de que tratam, então aposentar o curso tira os planos e turmas dele do ar sozinho — e reativar traz de volta. Quem aposenta e reativa é quem abre turma: `admin` e `enrollment_supervisor` (mais `master`); tudo vai pro `audit_log`. **Reativar existe de propósito** — sem ele um id errado seria irreversível, porque §6 não deixa DELETE e §7 não deixa SQL de mão em produção.
...
9. repositórios contra o banco — o SQL que o `tsc` não verifica (join na coluna errada compila) roda contra o Postgres migrado do mesmo job (`apps/api/**/*.integration.test.ts`, `pnpm test:api:db`)
